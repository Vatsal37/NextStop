...
export const search = asyncHandler(async (req, res) => {
	const { source, destination, date, page = 1, limit = 20 } = req.query;
	if (!source || !destination || !date) {
		throw new ApiError(400, 'source, destination and date are required');
	}
	const rows = await searchFlights({ sourceCode: source, destinationCode: destination, date, page: Number(page), limit: Number(limit) });
